...
void eval_period(struct task_par* tp, int* arr_tp, int* sel_t, int t_idx,
                 int* change_flag) {
 ...
 */
int thread_create(struct task_par* tp, struct sched_param* sp,
                  pthread_attr_t att, pthread_t* tid, void* task) {
  int ret = -1;

  pthread_attr_init(&att);
  pthread_attr_setinheritsched(&att, PTHREAD_EXPLICIT_SCHED);
  pthread_attr_setschedpolicy(&att, SCHED_OTHER);
 ...
 */
int mutex_create(pthread_mutex_t mux, pthread_mutexattr_t matt, int prot,
                 int ceiling) {
  int result = -1;
